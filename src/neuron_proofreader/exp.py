...
class CompactDGCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = x1.max(dim=-1)[0]                                       # pool over neighbors → (B, C1, N)
...
def index_points(points, idx):
    ...
    B = points.shape[0]
    S = idx.shape[1]
    batch_indices = torch.arange(B, dtype=torch.long, device=points.device).view(B, 1).repeat(1, S)
...
class PointNet2(nn.Module):
    def __init__(self, output_dim=1):
        super().__init__()
        self.sa1 = PointNetSetAbstraction(n_points=512, mlp_channels=[64, 64, 128])
        self.sa2 = PointNetSetAbstraction(n_points=128, mlp_channels=[128, 128, 256])
        self.sa3 = PointNetSetAbstraction(n_points=None, mlp_channels=[256, 512, 1024])  # global
...
class VisionPointNetModel(nn.Module):
