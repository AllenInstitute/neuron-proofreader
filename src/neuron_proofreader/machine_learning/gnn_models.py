...
class VisionHGT(torch.nn.Module):
    """
    Heterogeneous graph attention network that processes multimodal features
    such as image patches and feature vectors.
    """
    ...
    def __init__(
        ...
        layers = nn.ModuleList()
        for _ in range(n_layers):
            layers.append(HGTLayer(hidden_dim, heads=heads))
...
# --- Layers ---
class TwoHopAttentionLayer(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        q = self.q(x).view(-1, self.heads, self.dh)
        k = self.k(x).view(-1, self.heads, self.dh)
        v = self.v(x).view(-1, self.heads, self.dh)

        # gather messages
        q_i = q[row]   # target nodes
        k_j = k[col]   # source nodes
        v_j = v[col]

        # attention scores
        attn = (q_i * k_j).sum(-1) / (self.dh ** 0.5)

        # normalize per target node
        attn = softmax_per_row(attn, row)

        msg = attn.unsqueeze(-1) * v_j

        out = torch.zeros_like(q)
        out = out.index_add(0, row, msg)
